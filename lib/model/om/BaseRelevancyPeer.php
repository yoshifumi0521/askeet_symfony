...
abstract class BaseRelevancyPeer {
	...
	const CLASS_DEFAULT = 'lib.model.Relevancy';
	...
	{
		include_once 'lib/model/map/RelevancyMapBuilder.php';
		return BasePeer::getMapBuilder('lib.model.map.RelevancyMapBuilder');
	}
	...
	{
		return str_replace(RelevancyPeer::TABLE_NAME.'.', $alias.'.', $column);
	}
	...
	const COUNT = 'COUNT(ask_relevancy.ANSWER_ID)';
	const COUNT_DISTINCT = 'COUNT(DISTINCT ask_relevancy.ANSWER_ID)';
		...
		while($rs->next()) {
			...
			foreach($results as $temp_obj1) {
				$temp_obj2 = $temp_obj1->getAnswer(); 				if ($temp_obj2->getPrimaryKey() === $obj2->getPrimaryKey()) {
					$newObject = false;
										$temp_obj2->addRelevancy($obj1); 					break;
				}
			}
			if ($newObject) {
				$obj2->initRelevancys();
		...
		while($rs->next()) {
			...
			for ($j=0, $resCount=count($results); $j < $resCount; $j++) {
				...
				$temp_obj3 = $temp_obj1->getUser(); 				if ($temp_obj3->getPrimaryKey() === $obj3->getPrimaryKey()) {
					$newObject = false;
					$temp_obj3->addRelevancy($obj1); 					break;
				}
			}
	...
	public static function doUpdate($values, $con = null)
	{
		...
		if ($values instanceof Criteria) {
			...
			$comparison = $criteria->getComparison(RelevancyPeer::ANSWER_ID);
			$selectCriteria->add(RelevancyPeer::ANSWER_ID, $criteria->remove(RelevancyPeer::ANSWER_ID), $comparison);

			$comparison = $criteria->getComparison(RelevancyPeer::USER_ID);
			$selectCriteria->add(RelevancyPeer::USER_ID, $criteria->remove(RelevancyPeer::USER_ID), $comparison);
	...
	public static function doDeleteAll($con = null)
	{
	 ...
	 public static function doDelete($values, $con = null)
	 {
		...
		if ($values instanceof Criteria) {
			$criteria = clone $values; 		} elseif ($values instanceof Relevancy) {
			...
			$criteria->add(RelevancyPeer::ANSWER_ID, $vals[0], Criteria::IN);
			$criteria->add(RelevancyPeer::USER_ID, $vals[1], Criteria::IN);
		}
	...
	public static function doValidate(Relevancy $obj, $cols = null)
	{
		...
		$res =  BasePeer::doValidate(RelevancyPeer::DATABASE_NAME, RelevancyPeer::TABLE_NAME, $columns);
    if ($res !== true) {
        $request = sfContext::getInstance()->getRequest();
        foreach ($res as $failed) {
            $col = RelevancyPeer::translateFieldname($failed->getColumn(), BasePeer::TYPE_COLNAME, BasePeer::TYPE_PHPNAME);
	...
	public static function retrieveByPK( $answer_id, $user_id, $con = null) {
		...
		$criteria->add(RelevancyPeer::ANSWER_ID, $answer_id);
		$criteria->add(RelevancyPeer::USER_ID, $user_id);
		$v = RelevancyPeer::doSelect($criteria, $con);
...
if (Propel::isInit()) {
		...
		Propel::log('Could not initialize Peer: ' . $e->getMessage(), Propel::LOG_ERR);
			...
			require_once 'lib/model/map/RelevancyMapBuilder.php';
	Propel::registerMapBuilder('lib.model.map.RelevancyMapBuilder');
}
